...
- bsnr / lanr: je 9-stellige Ziffernfolge (Betriebsstätten-Nr. / Arzt-Nr.).
  WICHTIG: Nimm BSNR und LANR aus dem OBEREN Datenfeld des Rezepts (die Zeile mit
  "Betriebsstätten-Nr." und "Arzt-Nr.", direkt unter Kostenträgerkennung/Status).
  Dort stehen sie maschinell und vollständig 9-stellig. NICHT aus dem Arztstempel
  unten rechts nehmen - dort sind sie oft verkürzt, abgeschnitten oder schlecht
  lesbar. Beide bestehen NUR aus 9 Ziffern, keine Buchstaben. Wenn du im oberen
  Feld nur eine kürzere/längere Zahl liest, lies nochmal genau - es sind genau 9.
...
- zuzahlung: Die Zuzahlung steht in der SCHMALEN SENKRECHTEN SPALTE GANZ LINKS am
  Formularrand, direkt neben dem Kasten "Krankenkasse bzw. Kostenträger". Dort sind
  4 winzige Kästchen UNTEREINANDER, jeweils mit einem kleinen Label. Übliche
  Reihenfolge VON OBEN NACH UNTEN:
  1. (oberstes) = "Zuzahlungsfrei" / "gebührenfrei"
  2. (zweites)  = "Zuzahlungspflicht" / "gebührenpflichtig"
  3. (drittes)  = "Unfallfolgen"
  4. (unterstes)= "BVG"
  Lies das kleine Label am markierten Kästchen, soweit möglich. Schau GENAU, in
  welchem Kästchen ein X / Kreuz / Haken sitzt. Gib zurück:
  * "zuzahlungsfrei" wenn das Kästchen mit Label "...frei" markiert ist,
  * "zuzahlungspflichtig" wenn das Kästchen mit Label "...pflicht" markiert ist,
  * "" wenn nur Unfallfolgen/BVG markiert sind oder gar nichts.
  WICHTIG: Verlass dich primär auf das LABEL (frei vs. pflicht), nicht nur auf die
  Position - die Reihenfolge kann je nach Druck leicht variieren. Die Kästchen sind
  sehr klein, schau sorgfältig. Markierungen sind oft nur ein dünnes X.
...
def extrahiere_felder_ki(volltext: str = "", bild_pfad: str = None) -> dict:
    ...
    # Fallback aus dem OCR-Volltext: einzelne Felder ergänzen/korrigieren, die die
    # KI im Bild übersehen oder falsch gelesen hat. OCR (Apple Vision) und KI sind
    # zwei unabhängige Quellen – wo die eine patzt, fängt oft die andere ab.
    if volltext:
        import re as _re
        # 1) Versichertennummer (1 Großbuchstabe + 9 Ziffern)
        if not felder.get("versichertennummer", "").strip():
            treffer = _re.findall(r'\b([A-Z]\d{9})\b', volltext)
            if treffer:
                felder["versichertennummer"] = treffer[0]
                logger.info("Versichertennummer aus OCR ergänzt: %s", treffer[0])

        # 2) LANR / BSNR: wenn KI-Wert fehlt oder NICHT 9-stellig-numerisch ist,
        #    aus dem strukturierten OCR-Regex nachziehen (nutzt Label-Verankerung).
        def _ist9(v):
            return bool(v) and v.isdigit() and len(v) == 9
        if not _ist9(felder.get("lanr", "")) or not _ist9(felder.get("bsnr", "")):
            try:
                from core.ocr import extrahiere_felder as _ocr_felder
                ocr_d = _ocr_felder(volltext)
                if not _ist9(felder.get("lanr", "")) and _ist9(ocr_d.get("lanr", "")):
                    felder["lanr"] = ocr_d["lanr"]
                    logger.info("LANR aus OCR korrigiert: %s", ocr_d["lanr"])
                if not _ist9(felder.get("bsnr", "")) and _ist9(ocr_d.get("bsnr", "")):
                    felder["bsnr"] = ocr_d["bsnr"]
                    logger.info("BSNR aus OCR korrigiert: %s", ocr_d["bsnr"])
            except Exception as e:
                logger.warning("OCR-Fallback für LANR/BSNR fehlgeschlagen: %s", e)

        # 3) Diagnosegruppe: Wenn dort ein ICD-Code steht (z.B. "Q67.6G" statt "EX"),
        #    ist das Feld verrutscht. Verwerfen und echte Gruppe aus OCR nachziehen.
        GUELTIGE_GRUPPEN = {
            "WS","EX","CS","ZN","PN","AT","GE","LY","SO1","SO2","SO3","SO4","SO5",
            "SB1","SB2","SB3","EN1","EN2","EN3","PS1","PS2","PS3","PS4",
            "SC","RE1","RE2","SF","ST1","ST2","SP1","SP2","SP3","SP4","SP5","SP6",
        }
        dg = felder.get("diagnosegruppe", "").strip().upper()
        # Sieht der Wert wie ein ICD-Code aus? (Buchstabe + 2 Ziffern, evtl. .x)
        ist_icd_artig = bool(_re.match(r'^[A-Z]\d{2}(\.\d{1,3})?[GVAZ]?$', dg))
        if dg and (ist_icd_artig or dg not in GUELTIGE_GRUPPEN):
            # Wenn das ICD-Feld leer ist, aber hier ein ICD-Code steht: dorthin retten
            if ist_icd_artig and not felder.get("icd10", "").strip():
                felder["icd10"] = dg
                logger.info("ICD-Code '%s' aus Diagnosegruppen-Feld ins ICD-Feld übernommen", dg)
            echte_gruppe = ""
            try:
                from core.ocr import extrahiere_felder as _ocr_felder2
                ocr_dg = _ocr_felder2(volltext).get("diagnosegruppe", "").strip().upper()
                if ocr_dg in GUELTIGE_GRUPPEN:
                    echte_gruppe = ocr_dg
            except Exception:
                pass
            if echte_gruppe:
                logger.info("Diagnosegruppe '%s' (ICD-artig/ungültig) -> OCR: %s", dg, echte_gruppe)
                felder["diagnosegruppe"] = echte_gruppe
            elif ist_icd_artig:
                # Lieber leer als ein ICD-Code im Gruppenfeld (klarer Fehler statt Fehlbefund)
                logger.info("Diagnosegruppe '%s' sieht wie ICD aus -> geleert", dg)
                felder["diagnosegruppe"] = ""
    return felder
