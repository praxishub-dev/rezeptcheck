...
def pruefe_rezept(daten: dict) -> Pruefergebnis:
    ...
    # Heuristik: fehlende Pflichtfelder trotz vieler Treffer → wahrscheinlich OCR-Lesefehler.
    # Kern-Felder müssen erkannt sein: Heilmittel, Fachbereich, BSNR, Name
    kern_erkannt = all([
        daten.get("heilmittel"),
        daten.get("fachbereich"),
        daten.get("bsnr") or daten.get("lanr"),
        daten.get("patient_name") or daten.get("patient_vorname"),
    ])
    ocr_lesefehler = (
        not echte_fehler
        and len(fehlt_fehler) <= 4
        and erkannte_felder >= 12
        and kern_erkannt
    )

    if ocr_lesefehler:
        hinweise = [f.replace("Fehlt: ", "") + " nicht lesbar" for f in fehlt_fehler]
        warnungen.insert(0, "Bitte am Rezept gegenprüfen: " + ", ".join(hinweise))
        status = "GELB"
        fehler = []
    elif fehler:
        status = "ROT"
