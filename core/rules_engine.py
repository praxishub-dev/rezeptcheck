...
ICD_CODES_PATH = Path(__file__).parent / "icd10gm_codes.json"

# Amtliche ICD-10-GM-Codeliste (BfArM, abgeleitet) für Existenzprüfung.
_ICD_CODES_CACHE = None
_ICD_STAEMME_CACHE = None

def lade_icd_codes():
    """Lädt ICD-10-GM-Codeliste (Set) + 3-Steller-Stämme (Set).
    Quelle: BfArM ICD-10-GM (aufbereitetes Verzeichnis). Nur Codes, kein Klartext.
    """
    global _ICD_CODES_CACHE, _ICD_STAEMME_CACHE
    if _ICD_CODES_CACHE is None:
        try:
            with open(ICD_CODES_PATH, "r", encoding="utf-8") as f:
                liste = json.load(f)
            _ICD_CODES_CACHE = set(liste)
            _ICD_STAEMME_CACHE = set(c[:3] for c in _ICD_CODES_CACHE)
        except Exception:
            _ICD_CODES_CACHE = set()
            _ICD_STAEMME_CACHE = set()
    return _ICD_CODES_CACHE, _ICD_STAEMME_CACHE


def icd_existiert(code: str) -> bool:
    """Prüft, ob ein ICD-10-GM-Code existiert. Robust gegen Jahrgangs-Lücken:
    Ein 5-Steller gilt als existent, wenn sein 4-Steller-Stamm existiert (so
    erzeugen neue Sub-Codes neuerer Jahrgänge keinen Fehlalarm). Fantasie-Codes
    ohne existierenden Stamm fallen auf.
    """
    codes, staemme = lade_icd_codes()
    if not codes:
        return True  # Liste nicht ladbar -> Prüfung neutral (kein Fehlalarm)
    c = re.sub(r'[^A-Z0-9.]', '', (code or "").upper())
    c = re.sub(r'\s*[GVAZ]\s*$', '', c).strip()
    if not c:
        return False
    if c in codes:
        return True
    m = re.match(r'^([A-Z]\d{2})\.(\d)\d$', c)        # 5-Steller -> 4-Steller-Stamm
    if m and f"{m.group(1)}.{m.group(2)}" in codes:
        return True
    m = re.match(r'^([A-Z]\d{2})\.\d$', c)            # 4-Steller -> 3-Steller-Stamm
    if m and m.group(1) in codes:
        return True
    if re.match(r'^[A-Z]\d{2}$', c) and c in staemme: # 3-Steller
        return True
    return False
...
def pruefe_icd_feld(feld: str, diagnosegruppe: str) -> dict:
    ...
    # Existenzprüfung: formal gültige Codes, die NICHT in der ICD-10-GM existieren
    nicht_existent = [c for c in gueltige if not icd_existiert(c)]
...
def pruefe_rezept(daten: dict) -> Pruefergebnis:
    ...
    if not icd10:
        ...
        if not icd_pruef["gueltig"]:
            ...
            # Existenzprüfung gegen die amtliche ICD-10-GM-Liste
            for c in icd_pruef.get("nicht_existent", []):
                warnungen.append(
                    f"ICD-Code '{c}' nicht in der ICD-10-GM gefunden – Code am Rezept prüfen."
                )
    ...
    # Zuzahlung – KEINE ärztliche Pflichtangabe (§: Kreuz darf freigelassen werden,
    # Quelle: GKV-Praxis, der tatsächliche Status wird vom Leistungserbringer selbst
    # über Alter/Befreiungsausweis bestimmt). Daher KEIN ROT-Fehler, nur ein Hinweis,
    # wenn das Feld leer ist. Ein leeres Zuzahlungsfeld macht das Rezept NICHT ungültig.
    zuzahlung = daten.get("zuzahlung", "").strip().lower()
    if not zuzahlung:
        warnungen.append(
            "Zuzahlungsstatus nicht angekreuzt (keine ärztliche Pflichtangabe) – "
            "tatsächlichen Status selbst prüfen (Alter/Befreiungsausweis)."
        )
