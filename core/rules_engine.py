...
def pruefe_datum(wert: str) -> bool:
    ...
    return bool(re.match(r'^\d{2}\.\d{2}\.\d{4}$', wert.strip())) if wert else False
...
def pruefe_rezept(daten: dict) -> Pruefergebnis:
    ...
    # ── BLOCK 2: Arztdaten ───────────────────────────────────────────────────
    for feld_def in regeln["pflichtfelder"]["arztdaten"]:
        feld = feld_def["feld"]
        label = feld_def["label"]
        typ = feld_def["typ"]
        wert = daten.get(feld, "").strip()

        if typ in ("text", "vorhanden"):
            if not wert:
                fehler.append(f"Fehlt: {label}")
    ...
    # Stempel BSNR-Abgleich: BSNR im Stempel muss mit BSNR-Feld übereinstimmen
    bsnr_feld = daten.get("bsnr", "").strip()
    bsnr_stempel = daten.get("stempel_bsnr", "").strip()
    if bsnr_feld and bsnr_stempel and bsnr_feld != bsnr_stempel:
        fehler.append("BSNR im Stempel stimmt nicht mit BSNR-Feld überein")
