...
def scan_und_extrahiere(bild_pfad: "str | None" = None) -> dict:
    ...
    # Bild EINMAL aufbereiten (Verkleinern + Rotationskorrektur 0/90/180/270°).
    # Das aufgerichtete Bild geht sowohl an die OCR ALS AUCH an die KI – so liest
    # die KI auch kopfüber/seitlich gescannte Rezepte korrekt.
    bild_aufgerichtet = _verkleinere_bild(bild_pfad)
    volltext = _vision_text(bild_aufgerichtet)
    if not (volltext and len(volltext.strip()) > 80):
        volltext = _tesseract_text(bild_aufgerichtet)
    ...
    try:
        ...
        if ki_verfuegbar():
            felder = extrahiere_felder_ki(volltext, bild_pfad=bild_aufgerichtet)
            if felder:
                return felder
