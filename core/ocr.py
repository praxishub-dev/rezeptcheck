    ...
    if not bsnr:
        ...
        for z in such_b:
            ...
            if len(zwei) >= 2:
                ...
                lanr = zwei[0]
    if not ausstell:
        # Datum nach "Datum"-Label; "120.11.25" → führende Ziffer bei 3-stelligem Tag weg
        ...
        if hm_idx >= 0:
            for z in zeilen[hm_idx + 1:hm_idx + 5]:
                ...
                if (len(zs) > 4 and re.search(r'[A-Za-zäöü]', zs)
                        and not re.search(r'Heilmittel|Behandlungseinheit|Ergänzend|Maßgabe', zs, re.IGNORECASE)):
                    heilmittel = zs
                    break
