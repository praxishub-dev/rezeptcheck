...
def _verkleinere_bild(bild_pfad: str) -> str:
    ...
    try:
        ...
        # Automatische Rotationserkennung: beste Ausrichtung via Buchstaben-Anteil
        bestes_bild = img
        bester_score = -1.0
        for winkel in [0, 90, 180, 270]:
            kandidat = img.rotate(winkel, expand=True)
            # Kleines Vorschau-Bild für schnellen OCR-Check
            w_check = min(800, kandidat.width)
            r_check = w_check / kandidat.width
            klein = kandidat.resize((w_check, int(kandidat.height * r_check)), Image.LANCZOS)
            tmp_c = tempfile.mktemp(suffix=".png")
            klein.save(tmp_c)
            try:
                rc = subprocess.run(
                    ["tesseract", tmp_c, "stdout", "-l", "deu", "--oem", "1", "--psm", "6"],
                    capture_output=True, text=True, timeout=8
                )
                txt = rc.stdout.strip()
                if not txt:
                    continue
                # Buchstaben-Anteil als Qualitäts-Score (korrekte Rotation → mehr lesbare Zeichen)
                buchstaben = sum(1 for c in txt if c.isalpha())
                score = buchstaben / max(len(txt), 1)
                ...
                pass
        img = bestes_bild
        logger.info(f"Beste Rotation gewählt (score={bester_score:.2f})")
...
def extrahiere_felder(volltext: str) -> dict:
    ...
    for kasse in kassen:
        if re.search(rf'\b{re.escape(kasse)}\b', text, re.IGNORECASE):
            ...
            break
    # OCR-Artefakte für Krankenkassen
    if not d["krankenkasse"]:
        if re.search(r'^B[3R]\s', text, re.MULTILINE):
            d["krankenkasse"] = "BKK"
        elif re.search(r'(EAN|BAN)\.?\s*Kostentr|Kostentr[äa]ger\s+Heilmittel', text, re.IGNORECASE):
            d["krankenkasse"] = "BKK"  # "2 EAN. Kostenträger" = BKK 24
    ...
    if not d["versichertennummer"]:
        ...
        if vn:
            ...
            # OCR lässt D weg: suche 9-stellige Zahl nach "Versicherten-Nr" Label
            m_vn = re.search(r'Versicherten[^\n]{0,20}\n[^\n]*?(\d{9})', text, re.IGNORECASE)
            if m_vn:
                d["versichertennummer"] = "D" + m_vn.group(1)[1:]  # D + letzten 8 Stellen
            else:
    ...
    if m:
        ...
        dm = re.search(r'[\|1I]?(\d{1,2}\.\d{2}\.\d{2,4})', text)
    ...
    # SB1 kommt oft als "--- " oder "-- " nach "Diagnose-"
    if not d["diagnosegruppe"]:
        ...
        elif re.search(r'[Dd]iagnose-\s*[-–—]+', text):
            # Diagnosegruppe aus Kontext ableiten
            if re.search(r'Psoriasis|L40', text, re.IGNORECASE):
                d["diagnosegruppe"] = "SB1"
            elif re.search(r'Zerebralparese|Paraparese|G80|G82', text, re.IGNORECASE):
                d["diagnosegruppe"] = "ZN"
    ...
    if m:
        ...
        ls = "a"
    elif re.search(r'sch[äa]digung|st[öo]rung|funktion|patientenindividuelle', text, re.IGNORECASE):
