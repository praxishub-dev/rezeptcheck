...
def _verkleinere_bild(bild_pfad: str) -> str:
    ...
    try:
        ...
        # Automatische Rotationserkennung: bekannte Formular-Wörter als Score
        # Diese Wörter kommen auf jedem Muster-13-Rezept vor
        FORMULAR = [
            "Heilmittel", "Diagnose", "Versicherten", "Kostentr",
            "Krankenkasse", "Hausbesuch", "Therapie", "Behandlung",
            "Physiotherapie", "Ergotherapie", "BLANKO", "Leitsymptom"
        ]
        bestes_bild = img
        bester_score = -1
        for winkel in [0, 90, 180, 270]:
            kandidat = img.rotate(winkel, expand=True)
            w_check = min(1000, kandidat.width)
